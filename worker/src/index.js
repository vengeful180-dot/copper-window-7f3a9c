...
function allowedOrigins(env) {
  return [env.ALLOWED_ORIGIN].filter(Boolean).map((origin) => origin.replace(/\/$/u, ""));
}
...
function requestIp(request) {
  return (request.headers.get("CF-Connecting-IP") || request.headers.get("x-forwarded-for") || "unknown").slice(0, 64);
}

async function requireEdgeRate(limiter, key) {
  if (!limiter || typeof limiter.limit !== "function") return;
  const result = await limiter.limit({ key });
  if (!result?.success) throw new InputError("Too many attempts. Try again later.", 429);
}

async function requireSite(request, env, { countWrite = true } = {}) {
  requireRate(checkInviteRate(request));
  if (countWrite) requireRate(checkWriteRate(request));
  await requireEdgeRate(env.AUTH_RATE_LIMITER, `site:${requestIp(request)}`);
  if (countWrite) await requireEdgeRate(env.WRITE_RATE_LIMITER, `site:${requestIp(request)}`);
...
async function requireAccount(request, env, { countWrite = true } = {}) {
  ...
  await requireEdgeRate(env.ACCOUNT_RATE_LIMITER, `account:${account.accountId}`);
  if (countWrite) await requireEdgeRate(env.WRITE_RATE_LIMITER, `account:${account.accountId}`);
  return account;
}
...
async function requireAdmin(request, env) {
  ...
  await requireEdgeRate(env.ACCOUNT_RATE_LIMITER, `admin:${requestIp(request)}`);
  await requireEdgeRate(env.WRITE_RATE_LIMITER, "admin");
}
  ...
  return {
    async fetch(request, env) {
      ...
      try {
        ...
        if (request.method === "GET" && url.pathname === "/bootstrap/kdf") {
          await requireEdgeRate(env.AUTH_RATE_LIMITER, `kdf:${requestIp(request)}`);
          const file = await store().get("data/config.enc.json");
          const kdf = file?.document?.kdf;
          if (!kdf || kdf.name !== "PBKDF2" || kdf.hash !== "SHA-256" || !Number.isInteger(kdf.iterations) || typeof kdf.salt !== "string") {
            throw new GitHubError("Protected configuration is invalid.", 502);
          }
          return json(request, env, { kdf });
        }
        ...
        if (request.method === "POST" && url.pathname === "/api/account/lookup") {
          await requireEdgeRate(env.AUTH_RATE_LIMITER, `account:${requestIp(request)}`);
        ...
        if (request.method === "POST" && url.pathname === "/api/admin/session") {
          await requireEdgeRate(env.AUTH_RATE_LIMITER, `admin:${requestIp(request)}`);
          const rate = checkAdminRate(request);
          if (!rate.allowed) return json(request, env, { error: "Too many attempts. Try again later." }, 429, { "Retry-After": String(rate.retryAfter) });
          const body = validateAdminLoginBody(await readJsonBody(request));
