...
test("health endpoint is public but reveals no configuration", async () => {
  ...
  assert.match(response.headers.get("permissions-policy"), /camera=\(\), microphone=\(\)/u);
});

test("edge rate limiting blocks bootstrap work before the private repository is read", async () => {
  resetRateLimitsForTests();
  const env = await workerEnv();
  env.AUTH_RATE_LIMITER = { limit: async () => ({ success: false }) };
  let repositoryRead = false;
  const worker = createWorker(async () => { repositoryRead = true; throw new Error("should not run"); });
  const response = await worker.fetch(jsonRequest("/bootstrap/kdf", { method: "GET" }), env);
...
test("unexpected request fields are rejected", async () => {
  ...
  const env = await workerEnv();
  const worker = createWorker(async () => { throw new Error("unused"); });
  const lookup = await worker.fetch(jsonRequest("/api/account/lookup", { body: { name: "Test User", extra: true } }), env);
  assert.equal(lookup.status, 400);
  const admin = await worker.fetch(jsonRequest("/api/admin/session", { body: { password: "admin-test-password", extra: true } }), env);
...
test("Worker-backed reads return fresh encrypted repository files", async () => {
  ...
  const kdf = await worker.fetch(jsonRequest("/bootstrap/kdf", { method: "GET" }), env);
  assert.equal(kdf.status, 200);
  assert.deepEqual((await kdf.json()).kdf, config.kdf);

  const anonymousBootstrap = await worker.fetch(jsonRequest("/bootstrap/config", { method: "GET" }), env);
  assert.equal(anonymousBootstrap.status, 401);
  const bootstrap = await worker.fetch(jsonRequest("/bootstrap/config", { method: "GET", authorization: "Bearer site-test-token" }), env);
