...
const SHOUTOUT_PEOPLE_LIMIT = 20;
const BIRTHDAY_LIMIT = 100;
...
export function assertConfigRecord(config) {
  ...
  const rawShoutouts = config.shoutouts ?? { start: "", end: "", people: [] };
  if (!rawShoutouts || typeof rawShoutouts !== "object" || Array.isArray(rawShoutouts) || Object.keys(rawShoutouts).sort().join(",") !== "end,people,start" || !Array.isArray(rawShoutouts.people) || rawShoutouts.people.length > SHOUTOUT_PEOPLE_LIMIT) throw new Error("Invalid shoutouts.");
  const shoutoutPeople = [];
  const shoutoutNames = new Set();
  for (const value of rawShoutouts.people) {
    const name = normalizeName(value);
    const key = canonicalName(name);
    if (!name || name !== value || shoutoutNames.has(key)) throw new Error("Choose unique shoutout names.");
    shoutoutNames.add(key);
    shoutoutPeople.push(name);
  }
  const shoutoutStart = typeof rawShoutouts.start === "string" ? rawShoutouts.start.trim() : "";
  const shoutoutEnd = typeof rawShoutouts.end === "string" ? rawShoutouts.end.trim() : "";
  if (Boolean(shoutoutStart) !== Boolean(shoutoutEnd) || (shoutoutStart && (!isIsoDate(shoutoutStart) || !isIsoDate(shoutoutEnd) || shoutoutEnd < shoutoutStart))) throw new Error("Choose a valid shoutout period.");
  if (shoutoutPeople.length && !shoutoutStart) throw new Error("Choose a shoutout period.");

  if (config.birthdays !== undefined && (!Array.isArray(config.birthdays) || config.birthdays.length > BIRTHDAY_LIMIT)) throw new Error("Invalid birthdays.");
  const birthdayNames = new Set();
  const birthdays = (config.birthdays ?? []).map((birthday) => {
    if (!birthday || typeof birthday !== "object" || Array.isArray(birthday) || Object.keys(birthday).sort().join(",") !== "date,name") throw new Error("Invalid birthday.");
    const name = normalizeName(birthday.name);
    const key = canonicalName(name);
    if (!name || name !== birthday.name || !isIsoDate(birthday.date) || birthdayNames.has(key)) throw new Error("Each birthday needs a unique name and valid date.");
    birthdayNames.add(key);
    return { name, date: birthday.date };
  }).sort((first, second) => first.date.slice(5).localeCompare(second.date.slice(5)) || first.name.localeCompare(second.name));
  ...
  const links = config.links.map((link) => {
    ...
    if (url) {
      let parsed;
      try { parsed = new URL(url); } catch { throw new Error("Use a complete link beginning with https://."); }
      if (parsed.protocol !== "https:") throw new Error("Quick links must begin with https://.");
    }
    return { label, url };
  });
  ...
  return {
    ...
    groupName,
    mom: config.mom.trim(),
    shoutouts: { start: shoutoutStart, end: shoutoutEnd, people: shoutoutPeople },
    birthdays,
    links,
...
export function mergeConfigChanges(original, latest, desired) {
  ...
  for (const field of ["groupName", "mom", "shoutouts", "birthdays", "links", "officeDays"]) {
